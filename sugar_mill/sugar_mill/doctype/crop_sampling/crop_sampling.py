...
class CropSampling(Document):
    ...
    def on_submite(self):
        ...
        if len(existing_cropsampling)==0:
            if ((float(minimum_brix) <= float(self.average_brix) ) and (float(minimum_pairs)<= float(self.no_of_pairs))):
                ...
                frappe.msgprint(f"सॅम्पलिंग पूर्ण झाले")
                frappe.msgprint(f"परंतु {self.id} चा तोडणीसाठीचा कालावधी पूर्ण झालेला नाही.")
        else:
